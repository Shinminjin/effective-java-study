...
- `removeEldestEntry`에서 `size()`를 호출하여 맵 안의 원소 수를 알아내는 것은 `removeEledestEntry`가 인스턴스 메서드라 가능하다.
...
이미 자바 표준 라이브러리에는 다양한 용도의 표준 함수형 인터페이스를 제공하고 있다.
...
- Function 인터페이스에는 기본 타입을 반환하는 변형이 총 **9가지**가 있다.
    - SrtToResult 변형 - 입력과 결과 모두 기본 타입
